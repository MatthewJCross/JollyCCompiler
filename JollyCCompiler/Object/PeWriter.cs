...
namespace JollyCCompiler.Object
{
    public sealed class PeWriter
    {
        ...
        private static void WriteHintName(byte[] data, int offset, string name) { WriteUInt16(data, offset, 0); WriteAsciiString(data, offset + 2, name); }

        private static void WriteAsciiString(byte[] data, int offset, string value) { byte[] bytes = Encoding.ASCII.GetBytes(value); Buffer.BlockCopy(bytes, 0, data, offset, bytes.Length); data[offset + bytes.Length] = 0; }

        private static void PatchRipRelative32(byte[] data, int displacementOffset, uint nextInstructionRva, uint targetRva) { int displacement = checked((int)((long)targetRva - nextInstructionRva)); WriteInt32(data, displacementOffset, displacement); }

        private static void PatchRelative32(byte[] data, int displacementOffset, uint nextInstructionRva, uint targetRva) { int displacement = checked((int)((long)targetRva - nextInstructionRva)); WriteInt32(data, displacementOffset, displacement); }
        ...
        private static void WriteSection(FileStream stream, byte[] data, uint rawSize) { stream.Write(data, 0, data.Length); int padding = checked((int)rawSize - data.Length); if (padding > 0) stream.Write(new byte[padding], 0, padding); }
        ...
        private static int AlignUp(int value, int alignment) { return checked((value + alignment - 1) / alignment * alignment); }

        private static uint AlignUp(uint value, uint alignment) { return checked((value + alignment - 1) / alignment * alignment); }

        private static void AlignList(List<byte> buffer, int alignment) { while ((buffer.Count % alignment) != 0) buffer.Add(0); }
        ...
        private sealed class StartupEmitter
        {
            ...
            public void Mark(string label) { _labels[label] = _bytes.Count; }

            public int EmitRipCall() { _bytes.Add(0xFF); _bytes.Add(0x15); int offset = _bytes.Count; _bytes.AddRange(new byte[4]); return offset; }

            public int EmitRelativeCall() { _bytes.Add(0xE8); int offset = _bytes.Count; _bytes.AddRange(new byte[4]); return offset; }

            public void EmitJump(string label) { _bytes.Add(0xE9); int offset = _bytes.Count; _bytes.AddRange(new byte[4]); _patches.Add((offset, label, false)); }

            public void EmitConditionalJump(string label) { _bytes.Add(0x0F); _bytes.Add(0x84); int offset = _bytes.Count; _bytes.AddRange(new byte[4]); _patches.Add((offset, label, false)); }

            public byte[] Finish() { byte[] result = _bytes.ToArray(); foreach (var patch in _patches) { if (!_labels.TryGetValue(patch.Target, out int targetOffset)) throw new InvalidOperationException($"Unknown startup label '{patch.Target}'."); uint targetRva = TextRva + (uint)targetOffset; uint nextInstructionRva = TextRva + (uint)patch.Offset + 4; int displacement = checked((int)((long)targetRva - nextInstructionRva)); BinaryPrimitives.WriteInt32LittleEndian(result.AsSpan(patch.Offset, 4), displacement); } return result; }
        }
    }
}
...
//            if (result.MachineCode == null || result.MachineCode.Length == 0) throw new InvalidOperationException("Generated machine code is empty.");
//            string? directory = Path.GetDirectoryName(Path.GetFullPath(filePath));
//            if (!string.IsNullOrEmpty(directory)) Directory.CreateDirectory(directory);

//            const int entryStubSize = 68;
//            byte[] mainCode = result.MachineCode;
//            byte[] text = new byte[AlignUp(entryStubSize + mainCode.Length, (int)FileAlignment)];
//            int p = 0;
...
//            int getCommandLineCallOffset = p;
//            text[p++] = 0xFF;
//            text[p++] = 0x15;
//            int getCommandLineCallDispOffset = p;
//            p += 4;
...
//            int commandLineToArgvCallOffset = p;
//            text[p++] = 0xFF;
//            text[p++] = 0x15;
//            int commandLineToArgvCallDispOffset = p;
//            p += 4;
...
//            int mainCallOffset = p;
//            text[p++] = 0xE8;
//            int mainCallDispOffset = p;
//            p += 4;
...
//            int localFreeCallOffset = p;
//            text[p++] = 0xFF;
//            text[p++] = 0x15;
//            int localFreeCallDispOffset = p;
//            p += 4;
...
//            int exitProcessCallOffset = p;
//            text[p++] = 0xFF;
//            text[p++] = 0x15;
//            int exitCallDispOffset = p;
//            p += 4;

//            if (p != entryStubSize) throw new InvalidOperationException($"Startup stub size mismatch. Expected {entryStubSize}, generated {p}.");

//            Buffer.BlockCopy(mainCode, 0, text, entryStubSize, mainCode.Length);

//            byte[] rdata = BuildRDataSection(result, out Dictionary<string, uint> dataSymbolRvas);

//            byte[] idata = BuildImportSection(out uint getCommandLineIatRva, out uint commandLineToArgvIatRva, out uint localFreeIatRva, out uint printfIatRva, out uint exitProcessIatRva);

//            uint mainRva = TextRva + (uint)entryStubSize;

//            WriteRipRelative32(text, getCommandLineCallDispOffset, TextRva + (uint)(getCommandLineCallOffset + 6), getCommandLineIatRva);
//            WriteRipRelative32(text, commandLineToArgvCallDispOffset, TextRva + (uint)(commandLineToArgvCallOffset + 6), commandLineToArgvIatRva);
//            WriteRelative32(text, mainCallDispOffset, TextRva + (uint)(mainCallOffset + 5), mainRva);
//            WriteRipRelative32(text, localFreeCallDispOffset, TextRva + (uint)(localFreeCallOffset + 6), localFreeIatRva);
//            WriteRipRelative32(text, exitCallDispOffset, TextRva + (uint)(exitProcessCallOffset + 6), exitProcessIatRva);
...
//            const ushort numberOfSections = 3;
...
//            uint rdataVirtualSize = Math.Max(1u, (uint)rdata.Length);
//            uint idataVirtualSize = Math.Max(1u, (uint)idata.Length);
...
//            uint sizeOfImage = AlignUp(IDataRva + idataVirtualSize, SectionAlignment);

//            byte[] headers = new byte[headersSize];

//            WriteDosHeader(headers);
//            WritePeHeaders(headers, textVirtualSize, textRawSize, rdataVirtualSize, rdataRawSize, idataVirtualSize, idataRawSize, headersSize, sizeOfImage, numberOfSections, sizeOfOptionalHeader);
//            WriteSectionHeaders(headers, textVirtualSize, textRawSize, textRawPointer, rdataVirtualSize, rdataRawSize, rdataRawPointer, idataVirtualSize, idataRawSize, idataRawPointer);
...
//        private byte[] BuildRDataSection(X64CodeGenerationResult result, out Dictionary<string, uint> symbolRvas)
//        {
...
//        private byte[] BuildImportSection(out uint getCommandLineIatRva, out uint commandLineToArgvIatRva, out uint localFreeIatRva, out uint printfIatRva, out uint exitProcessIatRva)
//        {
//            const int kernel32IltOffset = 0x80;
//            const int shell32IltOffset = 0xA0;
//            const int msvcrtIltOffset = 0xC0;
//            const int kernel32IatOffset = 0xE0;
//            const int shell32IatOffset = 0x100;
//            const int msvcrtIatOffset = 0x120;
//            const int getCommandLineHintNameOffset = 0x130;
//            const int commandLineToArgvHintNameOffset = 0x150;
//            const int localFreeHintNameOffset = 0x170;
//            const int printfHintNameOffset = 0x180;
//            const int exitProcessHintNameOffset = 0x190;
//            const int kernel32NameOffset = 0x1A0;
//            const int shell32NameOffset = 0x1B0;
//            const int msvcrtNameOffset = 0x1C0;
//            const int size = 0x1D0;
...
//            localFreeIatRva = IDataRva + kernel32IatOffset + 8;
//            exitProcessIatRva = IDataRva + kernel32IatOffset + 16;
...
//            WriteImportDescriptor(data, 0x00, kernel32IltRva, kernel32NameRva, getCommandLineIatRva);
//            WriteImportDescriptor(data, 0x14, shell32IltRva, shell32NameRva, commandLineToArgvIatRva);
//            WriteImportDescriptor(data, 0x28, msvcrtIltRva, msvcrtNameRva, printfIatRva);

//            WriteUInt64(data, kernel32IltOffset + 0, getCommandLineHintNameRva);
//            WriteUInt64(data, kernel32IltOffset + 8, localFreeHintNameRva);
//            WriteUInt64(data, kernel32IltOffset + 16, exitProcessHintNameRva);
//            WriteUInt64(data, kernel32IltOffset + 24, 0);
...
//            WriteUInt64(data, kernel32IatOffset + 8, localFreeHintNameRva);
//            WriteUInt64(data, kernel32IatOffset + 16, exitProcessHintNameRva);
//            WriteUInt64(data, kernel32IatOffset + 24, 0);
...
//        private static void WriteRipRelative32(byte[] data, int displacementOffset, uint nextInstructionRva, uint targetRva)
//        {
//            int displacement = checked((int)((long)targetRva - nextInstructionRva));
//            WriteInt32(data, displacementOffset, displacement);
//        }

//        private static void WriteRelative32(byte[] data, int displacementOffset, uint nextInstructionRva, uint targetRva)
//        {
//            int displacement = checked((int)((long)targetRva - nextInstructionRva));
//            WriteInt32(data, displacementOffset, displacement);
//        }
...
//        private static void WritePeHeaders(byte[] headers, uint textVirtualSize, uint textRawSize, uint rdataVirtualSize, uint rdataRawSize, uint idataVirtualSize, uint idataRawSize, uint sizeOfHeaders, uint sizeOfImage, ushort numberOfSections, ushort sizeOfOptionalHeader)
//        {
//            int peOffset = 0x80;
...
//        private static void WriteSectionHeaders(byte[] headers, uint textVirtualSize, uint textRawSize, uint textRawPointer, uint rdataVirtualSize, uint rdataRawSize, uint rdataRawPointer, uint idataVirtualSize, uint idataRawSize, uint idataRawPointer)
//        {
