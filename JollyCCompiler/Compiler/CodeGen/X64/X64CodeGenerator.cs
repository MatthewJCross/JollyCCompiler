...
namespace JollyCCompiler.Compiler.CodeGen
{
    public sealed class X64CodeGenerator
    {
        ...
        private static bool TryGetScalarStorageOffset(string name, Dictionary<string, int> variables, Dictionary<string, (int Index, string Type)> parameters, out int offset)
        {
            if (variables.TryGetValue(name, out offset))
                return true;
            if (parameters.TryGetValue(name, out var parameter))
            {
                if (parameter.Type.EndsWith("*", StringComparison.Ordinal))
                    throw new NotSupportedException($"Operator on pointer parameter '{name}' is not yet supported.");
        ...
        {
            if (!TryGetScalarStorageOffset(expression.Name, variables, parameters, out var offset))
                throw new InvalidOperationException($"Variable or parameter '{expression.Name}' has no stack slot.");
            if (expression.Operator == TokenKind.Equals)
            {
            ...
            switch (expression.Operator)
            {
                ...
                case TokenKind.PercentEquals:
                    ...
                    break;
                default:
                    throw new NotSupportedException($"Assignment operator '{expression.Operator}' is not supported.");
            }
            emitter.MovRbpDisp8Eax(offset);
        }
        ...
        private static void GenerateUnaryExpression(UnaryExpression expression, X64Emitter emitter, List<X64DataItem> data, Dictionary<string, int> variables, Dictionary<string, (int Index, string Type)> parameters)
        {
            ...
            {
                GenerateIncrementDecrement(expression, emitter, variables, parameters);
                return;
            }
        ...
        private static void GenerateIncrementDecrement(UnaryExpression expression, X64Emitter emitter, Dictionary<string, int> variables, Dictionary<string, (int Index, string Type)> parameters)
        {
            if (expression.Operand is not IdentifierExpression identifier)
                throw new NotSupportedException("Increment and decrement operators currently require an identifier operand.");
            if (!TryGetScalarStorageOffset(identifier.Name, variables, parameters, out var offset))
